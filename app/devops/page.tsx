...
export default function DevOpsPage() {
    ...
    const fetchProjects = async (page = 1, searchQuery = search, category = activeCategory, showLoading = true) => {
        try {
            if (showLoading) setLoading(true);
            const params = new URLSearchParams({
                page: page.toString(),
                search: searchQuery,
                techStack: category === "All Projects" ? "" : category,
                limit: "9"
            });

            const res = await axios.get(`/api/devops?${params.toString()}`);
            if (res.data) {
    ...
    useEffect(() => {
        fetchProjects(1, search, activeCategory);
    }, [activeCategory]);
    ...
    const handlePageChange = (page: number) => {
        fetchProjects(page, search, activeCategory);
        window.scrollTo({ top: 0, behavior: "smooth" });
    };

    // Keep some static categories for the UI
    const categories = ["All Projects", "React", "Next.js", "Node.js", "Python", "Docker", "Kubernetes", "AWS"];
